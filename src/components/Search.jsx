...
import { createPortal } from 'react-dom'; // <--- IMPORTANTE: Importamos el Portal
...
export default function Search() {
  ...
  const openModal = () => {
    setIsOpen(true);
    // Bloquear el scroll de la página cuando se abre el buscador
  ...
  // Contenido del Modal (lo separamos para pasarlo al Portal)
  const modalContent = (
    <div 
      className="fixed inset-0 z-[99999] flex items-start justify-center pt-24 bg-black/60 backdrop-blur-sm p-4"
      onClick={(e) => {
         // Si haces clic en el fondo oscuro (y no en la caja blanca), se cierra
    ...
    >
      <div className="bg-white dark:bg-[#161b22] w-full max-w-2xl rounded-xl shadow-2xl border border-gray-200 dark:border-gray-700 overflow-hidden transform transition-all animate-fadeIn">
        ...
        <div className="flex items-center border-b border-gray-200 dark:border-gray-700 p-4">
          ...
          <button 
            onClick={closeModal}
            className="text-xs bg-gray-200 dark:bg-gray-700 px-2 py-1 rounded text-gray-500 dark:text-gray-300 hover:bg-gray-300 dark:hover:bg-gray-600 transition-colors"
          >
            ESC
          </button>
        </div>
  ...
  return (
    <>
      {/* BOTÓN (Se renderiza normal en la navbar) */}
      <button
        onClick={openModal}
      ...
      {/* MODAL (Se "teletransporta" al body gracias a createPortal) */}
